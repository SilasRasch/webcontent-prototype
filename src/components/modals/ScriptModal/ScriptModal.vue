...
<template>
    <a v-if="auth.isAdmin()" class="p-4 rounded-lg cursor-pointer text-nowrap w-full" :class="link === '' ? 'bg-slate-800' : 'bg-green-600'" @click="handleToggle">{{ props.name }}</a>
            ...
            <div v-show="showModal" @click.self="handleToggle" class="z-50 absolute bg-black bg-opacity-50 w-full h-full top-0 left-0">
                <div class="p-2 px-4 rounded-lg text-white min-w-[20rem] bg-gray-800 shadow-2xl shadow-black pb-4 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute">
                    <div class="grid w-full mt-2 gap-2">
                        ...
                        <input v-if="model.status.category === 1" placeholder="https://drive.google.com/..." class="input rounded-lg" v-model="link" />
                        <div v-else class="flex items-center">
                            <input placeholder="https://drive.google.com/..." class="input rounded-l-lg" v-model="link" />
